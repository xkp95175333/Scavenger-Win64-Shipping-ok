...
struct UFoliageInstancedStaticMeshComponent : UHierarchicalInstancedStaticMeshComponent {
	struct FMulticastInlineDelegate OnInstanceTakePointDamage; // 0x680(0x10)
	struct FMulticastInlineDelegate OnInstanceTakeRadialDamage; // 0x690(0x10)
	struct FGuid GenerationGuid; // 0x6a0(0x10)
};
...
struct UFoliageStatistics : UBlueprintFunctionLibrary {
	...
	int32_t FoliageOverlappingSphereCount(struct UObject* WorldContextObject, struct UStaticMesh* StaticMesh, struct FVector CenterPosition, float Radius); // Function Foliage.FoliageStatistics.FoliageOverlappingSphereCount // (Final|Native|Static|Public|HasDefaults|BlueprintCallable) // @ game+0xffff8008e4a10000
	int32_t FoliageOverlappingBoxCount(struct UObject* WorldContextObject, struct UStaticMesh* StaticMesh, struct FBox Box); // Function Foliage.FoliageStatistics.FoliageOverlappingBoxCount // (Final|Native|Static|Public|HasDefaults|BlueprintCallable) // @ game+0xffff8008e4a10000
};
...
struct UFoliageType : UObject {
	...
	bool bIncludeInLevelLOD; // 0x3a5(0x01)
	char pad_3A6[0x2]; // 0x3a6(0x02)
	float LevelLODPercentage; // 0x3a8(0x04)
...
struct UFoliageType_Actor : UFoliageType {
	struct AActor* ActorClass; // 0x3b0(0x08)
	bool bShouldAttachToBaseComponent; // 0x3b8(0x01)
...
struct UFoliageType_InstancedStaticMesh : UFoliageType {
	struct UStaticMesh* Mesh; // 0x3b0(0x08)
	struct TArray<struct UMaterialInterface*> OverrideMaterials; // 0x3b8(0x10)
	struct UFoliageInstancedStaticMeshComponent* ComponentClass; // 0x3c8(0x08)
};
...
struct AInteractiveFoliageActor : AStaticMeshActor {
	...
	struct FVector TouchingActorEntryPosition; // 0x2a0(0x0c)
	struct FVector FoliageVelocity; // 0x2ac(0x0c)
	struct FVector FoliageForce; // 0x2b8(0x0c)
	struct FVector FoliagePosition; // 0x2c4(0x0c)
	float FoliageDamageImpulseScale; // 0x2d0(0x04)
	float FoliageTouchImpulseScale; // 0x2d4(0x04)
	float FoliageStiffness; // 0x2d8(0x04)
	float FoliageStiffnessQuadratic; // 0x2dc(0x04)
	float FoliageDamping; // 0x2e0(0x04)
	float MaxDamageImpulse; // 0x2e4(0x04)
	float MaxTouchImpulse; // 0x2e8(0x04)
	float MaxForce; // 0x2ec(0x04)
	float Mass; // 0x2f0(0x04)
	char pad_2F4[0x4]; // 0x2f4(0x04)

	void CapsuleTouched(struct UPrimitiveComponent* OverlappedComp, struct AActor* Other, struct UPrimitiveComponent* OtherComp, int32_t OtherBodyIndex, bool bFromSweep, struct FHitResult OverlapInfo); // Function Foliage.InteractiveFoliageActor.CapsuleTouched // (Final|Native|Protected|HasOutParms) // @ game+0xffff8008e4a10000
};
...
struct UProceduralFoliageSpawner : UObject {
	...
	void Simulate(int32_t NumSteps); // Function Foliage.ProceduralFoliageSpawner.Simulate // (Final|Native|Public|BlueprintCallable) // @ game+0xffff8008e4a10000
};
...
struct AProceduralFoliageVolume : AVolume {
	struct UProceduralFoliageComponent* ProceduralComponent; // 0x2c0(0x08)
};
